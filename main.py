...
if __name__ == '__main__':
    mygen = Generator()
    print("设置题目难度\n")
    numofexp = int(input("请输入题目数量:\t"))
    myrange = int(input("请输入数字范围:\t"))
    hasfraction = input("式子中是否有分数(Y or N):\t").upper()
    hasmulanddiv = input("式子中是否包含乘除法(Y or N):\t").upper()
    hasparentheses = input("式子中是否带有括号(Y or N):\t").upper()
    hasneg = input("式子中是否有负数(Y or N):\t").upper()
    maxnumofoper = int(input("请输入运算符数量:(目前最多支持3种)\t"))
    if maxnumofoper >3:
        print("请重新输入运算符数量")
        maxnumofoper = input("请输入运算符数量:(目前最多支持3种)\t")
